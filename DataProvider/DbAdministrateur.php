...
class DbAdministrateur
{
    ...
    public static function checkOldPass($oldPass)
    {

        $bdd = PdoHelper::getInstance();
        //on prepare la requete
        $req = $bdd->DataBase->prepare("SELECT MotDePasse 
        FROM `Administrateur` WHERE  MotDePasse=:oldPass");

        $req->execute(array(':oldPass' => $oldPass));

        $nbre_lignes = $req->rowCount();

            while ($ligne = $req->fetch(PDO::FETCH_ASSOC)) {
                $password = $ligne['MotDePasse'];
            }
    ...
    public static function GetUtilisateur($utilisateurId)
    {
        try {
            $bdd = PdoHelper::getInstance();
            //on prepare la requete
            $req = $bdd->DataBase->prepare('SELECT Administrateur.Nom, Administrateur.Prenom, Administrateur.Identifiant, lower(Administrateur.Mail) as Mail, Administrateur.Actif, Administrateur.SuperAdmin, Administrateur.OPSNId, OPSN.Nom as NomOPSN, Administrateur.Id 
            FROM Administrateur, OPSN
            WHERE Administrateur.Id = :utilisateurId
            AND OPSN.Id = Administrateur.OPSNId');
            ...
            $utilisateur->NomOPSN = AjaxHelper::ToUtf8($utilisateurDb->NomOPSN);
    ...
    public static function GetIdDisponibility($Identifiant)
    {
        try {
            ...
            //on prepare la requete
            $bdd->DataBase->exec("SET CHARACTER SET utf8");
            $req = $bdd->DataBase->prepare("SELECT Identifiant FROM `Administrateur` 
    ...
    public static function SetMdp($Id, $password)
    {
        try {
            ...
            if ($success == 1) {
                $req = $bdd->DataBase->prepare("UPDATE `Administrateur` SET `MotDePasse`=:password, IdMotDePasseOublie=null, DateMotDePasseOublie=null WHERE IdMotDePasseOublie = :idMotDePasseOublie or Id = :id");
                $req->execute(array(':password' => $password,  ':id' => $Id, ':idMotDePasseOublie' => $Id));
            }
    ...
    public static function UpdateMotDePasseOublie($idMotDePasseOublie, $utilisateurId){
        try 
        { 
            $success = 1;
            ...
            if ($success == 1)
            {
                $bdd->DataBase->exec("SET CHARACTER SET utf8");
                $requete = ("UPDATE Administrateur
                SET IdMotDePasseOublie = :idMotDePasseOublie, DateMotDePasseOublie = now()
                WHERE Id = :utilisateurId");
            }
            
            $req = $bdd->DataBase->prepare($requete);
            $req->execute(array(":idMotDePasseOublie" => $idMotDePasseOublie,  ":utilisateurId" => $utilisateurId));
    ...
    public static function GetUtilisateurByMail($adresseMail)
    {
        try {
            $bdd = PdoHelper::getInstance();
            //on prepare la requete
            $req = $bdd->DataBase->prepare('SELECT Id, Nom, Prenom, Mail FROM Administrateur WHERE Mail = :adresseMail');
            ...
            $utilisateur->Nom = AjaxHelper::ToUtf8($utilisateurDb->Nom);
            $utilisateur->Prenom = AjaxHelper::ToUtf8($utilisateurDb->Prenom);
            $utilisateur->AdresseMail = AjaxHelper::ToUtf8($utilisateurDb->Mail);
    ...
    public static function CheckIdMotDePasseOublie($idMotDePasseOublie){

        $bdd = PdoHelper::getInstance();
        //on prepare la requete
        $req = $bdd->DataBase->prepare("SELECT Id as UserID 
        FROM `Administrateur`  
        WHERE  idMotDePasseOublie=:idmdpoublie 
        AND TIMESTAMPDIFF(MINUTE, 
        DateMotDePasseOublie, now())<2880");

        $req->execute(array( ':idmdpoublie' => $idMotDePasseOublie));
