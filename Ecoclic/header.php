<!DOCTYPE html>
<html lang="fr" style="background-color: #F2F2F2;">
    <head>
        <title>L'éco-clic</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css" integrity="sha384-9aIt2nRpC12Uk9gS9baDl411NQApFmC26EwAOH8WgZl5MYYxFfc+NcPb1dKGj7Sk" crossorigin="anonymous">
        <link rel="stylesheet" href="css/ecoclic.css">
        <link rel="stylesheet" href="css/menu.css">
        <link rel="stylesheet" href="css/recherche.css">
        <link rel="stylesheet" href="https://cdn3.devexpress.com/jslib/21.1.5/css/dx.light.css">
        <script src="https://kit.fontawesome.com/a94d7dd11a.js" crossorigin="anonymous"></script>

        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <script src="https://unpkg.com/devextreme-quill@1.5.16/dist/dx-quill.min.js"></script>
        <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
        <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.min.js" integrity="sha384-OgVRvuATP1z7JjHLkuOU7Xw704+h835Lr+6QL9UvYjZE3Ipu6Tp75j7Bh/kR0JKI" crossorigin="anonymous"></script>
        <script src="./devExtreme/js/dx.all.js"></script>
        <script src="./js/common.js"></script>
        <link rel="stylesheet" href="./devExtreme/css/dx.common.css" />
        <link rel="stylesheet" href="./devExtreme/css/Grid.adico.css" />
        <link rel="stylesheet" href="./devExtreme/css/dx.light.css" />
        <script src="./devExtreme/js/localization/dx.messages.fr.js"></script>
        <script>
            DevExpress.localization.locale(navigator.language);
            DevExpress.localization.loadMessages(navigator.language);
        </script>
        
    </head>
    <body>
        <div class="wrapper" style="overflow: hidden;">

        <script>
        </script>